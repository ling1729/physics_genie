...
class com_wiris_quizzes_impl_QuestionInstanceImpl extends com_wiris_util_xml_SerializableImpl implements com_wiris_util_type_Comparator, com_wiris_quizzes_api_QuestionInstance{
	public function __construct() {
		...
		$this->compoundGraphicalChecks = null;
	}}
	...
	public function getMatchingCompoundChecks($slot, $authorAnswer, $index) {
		...
		$checks = $this->getCompoundAnswerChecks($correctAnswer, $userAnswer, $index);
		if($checks === null) {
			return new _hx_array(array());
		} else {
	...
	public function getCompoundComponents() {
		if($this->compoundChecks !== null) {
			...
			if($this->compoundGraphicalChecks !== null) {
				$min = -1;
				$it = $this->compoundGraphicalChecks->keys();
				while($it->hasNext()) {
					$jt = $this->compoundGraphicalChecks->get($it->next())->keys();
					$count = 0;
					while($jt->hasNext()) {
						$jt->next();
						++$count;
					}
					$min = (($min === -1 || $count < $min) ? $count : $min);
					unset($jt,$count);
				}
				return $min;
			} else {
	...
	public function getCompoundAnswerChecks($correctAnswer, $studentAnswer, $index) {
		if($this->compoundChecks !== null) {
			...
			if($this->compoundGraphicalChecks !== null) {
				$h = $this->compoundGraphicalChecks->get(_hx_string_rec($studentAnswer, "") . "");
				$it = $h->keys();
				{
					$_g = 0;
					while($_g < $index) {
	...
	public function collapseCompoundAnswerChecks($checks) {
		...
		$i = null;
		{
			...
			while($_g1 < $_g) {
				...
				unset($pairs,$j,$idAnswer,$i1,$correctAnswers,$correctAnswerId,$c,$answers);
			}
		}
	}
	...
	public function collapseCompoundGraphicalAnswerChecks($checks) {
		...
		$this->compoundGraphicalChecks = new Hash();
		...
		while($it->hasNext()) {
			...
			$grade_prev = 0.0;
			{
				...
				while($_g1 < $_g) {
					...
					$c->setAnswer($answer);
					$c->setCorrectAnswer($correctAnswer);
					$elemId = $piece->get("elemId");
					if(!$this->compoundGraphicalChecks->exists($answer)) {
						$this->compoundGraphicalChecks->set($answer, new Hash());
					}
					if(!$this->compoundGraphicalChecks->get($answer)->exists($elemId)) {
						$this->compoundGraphicalChecks->get($answer)->set($elemId, new _hx_array(array()));
					}
					$this->compoundGraphicalChecks->get($answer)->get($elemId)->push($c);
					$grade_prev = $grade;
					...
					if($tag === com_wiris_quizzes_impl_ResultGetVariables::$tagName) {
						...
						if($tag === com_wiris_quizzes_impl_ResultGetCheckAssertions::$tagName) {
							...
							if($this->isCompoundGraphicalAnswerChecks($resultChecks)) {
								$this->collapseCompoundGraphicalAnswerChecks($resultChecks);
							}
	...
	public function onSerialize($s) {
		...
		if($this->isCompoundGraphicalAnswerChecks($a)) {
			$this->collapseCompoundGraphicalAnswerChecks($a);
		}
	...
	public $question;
	public $compoundGraphicalChecks;
