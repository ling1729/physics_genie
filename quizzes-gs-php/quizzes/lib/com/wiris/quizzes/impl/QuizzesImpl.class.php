...
class com_wiris_quizzes_impl_QuizzesImpl extends com_wiris_quizzes_api_Quizzes {
	...
	public function addInterestingLocalDataToRequest($q, $qi, $qr) {
		$sb = new StringBuf();
		$localDataQ = com_wiris_quizzes_impl_QuizzesImpl_0($this, $q, $qi, $qr, $sb);
		$localDataQI = com_wiris_quizzes_impl_QuizzesImpl_1($this, $localDataQ, $q, $qi, $qr, $sb);
		$localData = null;
		...
		}
		{
			...
			while($_g < $localData->length) {
				$ld = $localData[$_g];
				++$_g;
				if(com_wiris_util_type_Arrays::containsArray(com_wiris_quizzes_impl_QuizzesImpl::$INTERESTING_LOCAL_DATA_NAME_AND_VALUE, $ld->name)) {
					if(strlen($sb->b) > 0) {
						$sb->add(",");
					}
					$sb->add($ld->name . "=" . $ld->value);
				} else {
					if(com_wiris_util_type_Arrays::containsArray(com_wiris_quizzes_impl_QuizzesImpl::$INTERESTING_LOCAL_DATA_ONLY_NAME, $ld->name) && $ld->value !== null && strlen($ld->value) > 0) {
						if(strlen($sb->b) > 0) {
							$sb->add(",");
						}
						$sb->add($ld->name . "=true");
	...
	public function isGraphicalAssertion($a) {
		return $a->name === com_wiris_quizzes_impl_Assertion::$EQUIVALENT_GRAPHIC || $a->name === com_wiris_quizzes_impl_Assertion::$CHECK_COLOR || $a->name === com_wiris_quizzes_impl_Assertion::$CHECK_LINESTYLE || $a->name === com_wiris_quizzes_impl_Assertion::$CHECK_NO_SUPERFLUOUS;
	}
	public function breakCompoundGraphical($qq) {
		...
		$toRemove = new _hx_array(array());
		{
			...
			while($_g1 < $_g) {
				$i = $_g1++;
				if($this->isGraphicalAssertion($qq->getAssertion($i)) && $qq->getAssertion($i)->getParam(com_wiris_quizzes_impl_Assertion::$PARAM_ELEMENTS_TO_GRADE) !== null) {
					...
					$elements = com_wiris_util_json_JSon::getArray(com_wiris_util_json_JSon::decode($ass->getParam(com_wiris_quizzes_impl_Assertion::$PARAM_ELEMENTS_TO_GRADE)));
					$steppedElements = new _hx_array(array());
					{
						$_g3 = 0; $_g2 = $elements->length;
						while($_g3 < $_g2) {
							$j = $_g3++;
							$elemId = _hx_string_call($elements[$j], "toString", array());
							$steppedElements->push($elemId);
							$ca_j = new com_wiris_quizzes_impl_CorrectAnswer();
							$ca_j->type = com_wiris_quizzes_impl_MathContent::$TYPE_GEOMETRY_FILE;
							$ca_j->id = $ca->id . "_cg" . _hx_string_rec($j, "") . "_" . $elemId;
							$ca_j->content = $ca->content;
...
com_wiris_quizzes_impl_QuizzesImpl::$INTERESTING_LOCAL_DATA_NAME_AND_VALUE = new _hx_array(array(com_wiris_quizzes_impl_LocalData::$KEY_SHOW_AUXILIARY_TEXT_INPUT, com_wiris_quizzes_impl_LocalData::$KEY_SHOW_CAS));
com_wiris_quizzes_impl_QuizzesImpl::$INTERESTING_LOCAL_DATA_ONLY_NAME = new _hx_array(array(com_wiris_quizzes_impl_LocalData::$KEY_AUXILIARY_TEXT, com_wiris_quizzes_impl_LocalData::$KEY_CAS_SESSION, com_wiris_quizzes_impl_LocalData::$KEY_CAS_INITIAL_SESSION));
function com_wiris_quizzes_impl_QuizzesImpl_0(&$»this, &$q, &$qi, &$qr, &$sb) {
	if($q !== null) {
		return $q->getImpl()->localData;
	}
}
function com_wiris_quizzes_impl_QuizzesImpl_1(&$»this, &$localDataQ, &$q, &$qi, &$qr, &$sb) {
	if($qi !== null) {
		return $qi->localData;
	}
}
